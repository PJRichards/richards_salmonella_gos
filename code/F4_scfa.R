...
sig_scfa_pw <- scfa_pw %>% pull(SCFA) %>% unique()

# add placeholder/blank data to generate headspace for bars 
# indicating significance
scfa_placeholder <- tibble(SCFA = "Valerate",
                Concentration = 0.6,
                age = "22",
                cohort = "ctl x SE")
...
scfa_pw.p <- scfa_long %>% 
                      ...
                      ggplot(aes(x = age, y = Concentration, fill = cohort)) +
                        ...
                        geom_blank(data=scfa_placeholder) + 
...
# pull out p values
# butyrate
But_GOSxSE_ctlxMock_28 <- scfa_pw %>% filter(SCFA=="Butyrate", group1=="jGOS x SE", group2=="ctl x Mock", age=="28") %>% pull(sig)

# propionate
Prop_GOSxSE_ctlxSE_28 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="jGOS x SE", group2=="ctl x SE", age=="28") %>% pull(sig)
Prop_GOSxSE_ctlxMock_28 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="jGOS x SE", group2=="ctl x Mock", age=="28") %>% pull(sig)
Prop_GOSxSE_GOSxMock_28 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="jGOS x SE", group2=="jGOS x Mock", age=="28") %>% pull(sig)
Prop_ctlxSE_ctlxMock_28 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="ctl x SE", group2=="ctl x Mock", age=="28") %>% pull(sig)
Prop_GOSxMock_ctlxSE_28 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="jGOS x Mock", group2=="ctl x SE", age=="28") %>% pull(sig)

Prop_GOSxSE_ctlxMock_35 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="jGOS x SE", group2=="ctl x Mock", age=="35") %>% pull(sig)
Prop_GOSxSE_GOSxMock_35 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="jGOS x SE", group2=="jGOS x Mock", age=="35") %>% pull(sig)
Prop_ctlxSE_ctlxMock_35 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="ctl x SE", group2=="ctl x Mock", age=="35") %>% pull(sig)
Prop_GOSxMock_ctlxSE_35 <- scfa_pw %>% filter(SCFA=="Propionate", group1=="jGOS x Mock", group2=="ctl x SE", age=="35") %>% pull(sig)

# valerate
Val_GOSxSE_ctlxMock_28 <- scfa_pw %>% filter(SCFA=="Valerate", group1=="jGOS x SE", group2=="ctl x Mock", age=="28") %>% pull(sig)
Val_GOSxSE_ctlxSE_28 <- scfa_pw %>% filter(SCFA=="Valerate", group1=="jGOS x SE", group2=="ctl x SE", age=="28") %>% pull(sig)
Val_GOSxSE_GOSxMock_28 <- scfa_pw %>% filter(SCFA=="Valerate", group1=="jGOS x SE", group2=="jGOS x Mock", age=="28") %>% pull(sig)

Val_ctlxSE_ctlxMock_35 <- scfa_pw %>% filter(SCFA=="Valerate", group1=="ctl x SE", group2=="ctl x Mock", age=="35") %>% pull(sig)
Val_GOSxMock_ctlxSE_35 <- scfa_pw %>% filter(SCFA=="Valerate", group1=="jGOS x Mock", group2=="ctl x SE", age=="35") %>% pull(sig)
Val_GOSxSE_ctlxMock_35 <- scfa_pw %>% filter(SCFA=="Valerate", group1=="jGOS x SE", group2=="ctl x Mock", age=="35") %>% pull(sig)
Val_GOSxSE_GOSxMock_35 <- scfa_pw %>% filter(SCFA=="Valerate", group1=="jGOS x SE", group2=="jGOS x Mock", age=="35") %>% pull(sig)
...
abund_scfa <- left_join(
                      scfa_wide %>% select(cohort, replicate, Group, all_of(sig_scfa_pw)),
...
# at any time
abund_corr_keep <- abund_corr %>% 
                      filter(q <= 0.05) %>% 
                      pull(taxon) %>% 
                      unique()

# plot sig correlated taxa only
abund_corr.p <- abund_corr %>% filter(taxon %in% abund_corr_keep) %>% 
