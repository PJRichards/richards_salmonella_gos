...
Key words: prebiotic, GOS, galactooligosaccharide, chicken, propionic acid, 
propionate, SCFA, Salmonella, microbiome, microbiota
...
Text word count: 4794 words

## Abstract
Chicken meat is contaminated with *Salmonella* originating from the gut of infected 
chickens during slaughter. Eradication of *Salmonella* from broiler (meat) chickens 
through hygiene measures and/or vaccination is not cost-effective, so complementary 
approaches are sought. A mature gut microbiota obstructs *Salmonella* infection 
in chickens and deliberate fortification of colonization resistance through 
prebiotic feed formulations would benefit public health and the poultry industry. 
Prebiotic galactooligosaccharides have been shown to hasten clearance of *Salmonella* 
from the gut of infected chickens. To better understand the role of 
galactooligosaccharides in colonization resistance broiler chickens were raised 
on a wheat-soybean meal-based feed, with or without added galactooligosaccharide 
for the first 24-days of life. Chickens were then orally challenged with 
*Salmonella enterica* serovar Enteritidis at 20-days-old and the effect of 
supplementary galactooligosaccharides characterized by profiling *Salmonella* 
colonization, gut microbiota, innate immune response and cecal SCFA concentrations. 
Exposure to dietary galactooligosaccharides shortened the time to clear *S*. Enteritidis 
from the ceca. Differential abundance analysis of the cecal microbiota associated 
*Salmonella*-challenge with a bacterial taxon belonging to the Acidaminococcaceae 
family. Increased cecal concentrations of the short-chain fatty acids propionate 
and valerate were measured in *Salmonella*-challenged chickens sustained on either 
control or galactooligosaccharide-supplemented feed relative to mock-challenged 
controls; but far greater concentrations were detected in chickens fed a 
...
The absence of cecal proinflammatory transcriptional responses suggest that the 
rapid *Salmonella*-clearance observed for the galactooligosaccharide-supplemented 
...
colonizing the chicken gut produce excess levels of propionate. In the absence of 
gross inflammation propionate is inimical to *Salmonella* and hastens intestinal 
clearance.
...
By 4 dpi *Salmonella* was detected by enrichment from the spleens and livers of `r SE_enrich %>% filter(age==22) %>% pull(n) %>% unique()` of 
...
chickens from the control group and `r SE_enrich %>% filter(age==35,site=="Spleen", group=="Gp 4") %>% pull(pos)` of `r SE_enrich %>% filter(age==35,site=="Spleen", group=="Gp 4") %>% pull(n)` chickens from the jGOS 
group. 
...
cecal microbiota of broiler chickens was similarly affected by *Salmonella* challenge and 
...
on the control feed was unchanged by SE-challenge (*p* $\geq$ `r round(dunnflat %>% filter(Comparison=="ctl sal - ctl unc") %>% pull(P.adj) %>% min(),3)`, Figure 3A). 
...
SE- and mock-challenged chickens in the control-feed group (Figure 3A). Changes in
$\beta$-diversity are particularly evident from 8 dpi onwards. Correspondingly, the inverse 
Simpson index of SE-challenged chickens in the jGOS group was lower than that of 
all other treatment groups for 35-day-old birds (*p* $\leq$ `r round(dunnflat %>% filter(Age=="35 (15)", str_detect(Comparison, "gos sal")==TRUE) %>% pull(P.adj) %>% max(),3)`; Figure 3A). 
These data show that sustaining broiler chickens on a GOS supplemented diet up until 
they are 24 days old affects changes in the cecal microbiota caused by *Salmonella* 
...
observed in SE-challenged birds on the control diet (Figure 3F), alongside up-regulation 
of STAT3 (*p* = `r FC.stats %>% filter(Age==22, gene=="STAT3") %>% pull(p_adjust) %>% round(3)`) and TGIF1 (*p* = `r FC.stats %>% filter(Age==22, gene=="TGIF1") %>% pull(p_adjust) %>% round(3)`) (Figure S2). 

```{r serum, include=FALSE}
source("code/SF3_serum.R", echo=TRUE)
...
height at any time point (*p^adj^* $\geq$ `r min(villus_TukeyHSD$adj.p.value) %>% round(3)`, 
Figure 2C).  However, we note that *Salmonella* could be detected 
...
sustained on the control diet than mock-challenged jGOS chickens at 28 days 
(*p* = `r round(prop_28_ctlsalmVSjgosmock,3)`).

```{r OTU vs SCFA, include=FALSE}
# draw Supplementary Figure S4
source("code/SF4_scfa_OTU.R", echo=TRUE)
...
Oral gavage with *Salmonella* Enteritidis did not result in overt intestinal disease,
...
(`r propionate_28_mM %>% filter(cohort=="jGOS x SE") %>% pull(mean_mM) %>% round(2)` mM) 
relative to *Salmonella*-challenged chickens on the control diet (`r propionate_28_mM %>% filter(cohort=="ctl x SE") %>% pull(mean_mM) %>% round(2)` mM); establishing a link between SE challenge
and cecal propionate that is augmented by, but not dependent on, juvenile exposure 
to supplementary GOS. As supplementary GOS was 
...
*S.* Enteritidis [@vanImmerseel2003invasion]. Moreover, 3.13 mM 
...
propionate is heightened at the pH of the ceca or the niches occupied by 
*Salmonella*. 
...
correlated with both propionate and valerate concentration.  

We observed that *Salmonella* was more effectively 
...
the primary GOS degraders still to be determined.  

The 16S rRNA sequencing error rate, as calculated against a known mock community 
(positive control) indicates that the errors introduced during sequencing and analysis 
are within appropriate margins. However, although *Salmonella* Enteritidis was 
successfully cultured from the cecal contents of all infected chickens and all 
rarefaction curves approached asymptote, *Salmonella* was not detected using 16S 
rRNA amplicon profiling. It appears that the levels of *Salmonella* colonizing 
the gut were below the limit of detection of this approach. 
...
enables the response to *Salmonella* colonization. We believe that the specific 
...
*et al.* [@gibson2017prebiotic], and by doing so has potential as an attractive target for 
...
content at the expense of soya meal by 2 and 5% w/w respectively. Figure 1 
...
protein and Degussa poultry digestible amino acid.
...
were performed in the R language (version 4.3.2). Mothur (version 1.48) was used 
...
with days post infection in parentheses.
...
are calculated on the basis of `r abund_scfa |> filter(age=="22") %>% pull(Group) |> n_distinct("Group")` measurements at 22 days, `r abund_scfa |> filter(age=="24") %>% pull(Group) |> n_distinct("Group")`
measurements at 24, `r abund_scfa |> filter(age=="28") %>% pull(Group) |> n_distinct("Group")`
measurements at 28 days and `r abund_scfa |> filter(age=="35") %>% pull(Group) |> n_distinct("Group")` measurements at 35 days.
