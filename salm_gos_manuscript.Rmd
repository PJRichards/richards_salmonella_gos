...
taxon. The absence of ileal and cecal propionate proinflammatory chemokine/cytokine
...
by indigenous taxa colonizing the cecal pouch yields excess levels of propionate. 
...
suggest a common mechanistic role for propionate. Consumer demand for chicken
...
shape the juvenile and mature broiler chicken cecal microbiotas to affect the 
...
enumeration and SCFA analysis. Prior to incising the gut, portions of spleen and 
liver were aseptically collected to sterile polythene bags that were also stored 
...
# drop early birds
mass <- read_tsv(mass_raw_path)

mass %>% group_by(Age, Diet, Challenge) %>% count() %>% pull(n) %>% unique()
...
# pen 'n'
#pen_mass_n <- pen_mass %>% group_by(challenge, age, diet) %>% summarise(n = n())

#test mean pen mass data for significance
mass_TukeyHSD <- 
  mass %>% 
            group_by(Age) %>% 
            nest() %>%  
            mutate(Tukey = map(data, ~ TukeyHSD(aov(LBW ~ Challenge * Diet, data=.x)) %>% 
                              tidy())) %>% 
            select(-data) %>% 
            unnest(cols = Tukey) %>% 
...
mock-challenged chickens on the calorie-balanced control diet at `r mass_TukeyHSD %>% filter(adj.p.value < 0.05) |> pull(Age) |> first()`
and `r mass_TukeyHSD %>% filter(adj.p.value < 0.05) |> pull(Age) |> last()` days 
old (*p* $\leq$ `r mass_TukeyHSD %>% filter(adj.p.value < 0.05) |> pull(adj.p.value) |> max() |> round(3)`; Figure 2B), 
corresponding with the period during which supplementary GOS was added to the 
diet. Thereafter in the absence of the GOS supplement there was no significant 
difference in the body weights between any of the groups (28 and 35 days old), 
which included the SE-challenged birds that did not suffer any growth check.
These findings indicate that whilst dietary inclusion of GOS in early life 
hastened the clearance of *Salmonella*, the inclusion did not effect the 
body weight at 35 days old.
...
on the control feed was unchanged by SE-challenge (*p* $\geq$ `r round(unlist(alpha_stats$ctl) %>% min(), 3)`, Figure 3B). 
...
chickens relative to mock-challenged chickens from the jGOS group, and these differences
extend to both the SE- and mock-challenged chickens in the control-feed group, which 
...
all other treatment groups for 35 day-old birds (*p* $\leq$ `r round(unnest(alpha_stats[4,], cols = test) %>% filter(str_detect(Comparison, "gos sal")==TRUE) %>% pull(P.adj) %>% max(), 3)`; Figure 3B). 
...
```
By comparison with other packages forms of analysis, Nearing *et al.* [@nearing2022daamethods] showed 
...
All of the taxa that identified as as increasing in abundance in response to the 
...
*p*-value < 0.005) were reduced by the jGOS diet. 
...
*Salmonella*-challenge in chickens (Figures 3D and 3E), possibly due to the 
...
exclusively, the ileum and ceca are reported to involve IFN-$\gamma$, 
IL-17, IL-1$\beta$, IL-22 and IL-8 [@withanage2005cytokine; @crhanova2011immune; 
...
microbiota was associated with changes in host immunity. We did not observe any 
transcriptional responses >±2-fold for SE colonization compared with mock treatment 
groups. However, the supplementation of juvenile feed with GOS differentiated the 
...
sections of ileal and cecal tissues revealed no significant differences in villus 
...
in shortening the period during which *Salmonella* colonizes the cecal pouch is
...
```
Short chain fatty acids (SCFA) are produced in the gut through microbial breakdown 
...
butyrate levels in SE-challenged, jGOS group chickens (mean `r round(butyrate_28_jGOS_SE,1)` 
mM) were greater than unchallenged chickens raised on control feed (mean `r round(butyrate_28_Ctl_Mock,1)` 
...
```
A correlation analysis was undertaken on observations of the levels of propionate, 
butyrate and valerate and the abundance of OTUs in the ceca of each individual 
...
Acidaminococcaceae_unclassified (OTU0007) was correlated with propionate at 28 ($\rho$ = `r abund_corr %>% filter(taxon == "Acidaminococcaceae_unclassified (Otu0007)", age==28, SCFA == "Propionate") %>% pull(estimate) %>% round(.,3)`, *p^adj^* = `r abund_corr %>% filter(taxon == "Acidaminococcaceae_unclassified (Otu0007)", age=="28", SCFA == "Propionate") %>% pull(q) %>% round(.,3)`) and 35 days ($\rho$ = `r abund_corr %>% filter(taxon == "Acidaminococcaceae_unclassified (Otu0007)", age=="35", SCFA == "Propionate") %>% pull(estimate) %>% round(.,3)`, *p^adj^* = `r abund_corr %>% filter(taxon == "Acidaminococcaceae_unclassified (Otu0007)", age == "35", SCFA == "Propionate") %>% pull(q) %>% round(.,3)`). Acidaminococcaceae_unclassified (OTU0007) 
was also positively correlated with valerate at 35 days ($\rho$ = `r abund_corr %>% filter(taxon == "Acidaminococcaceae_unclassified (Otu0007)", age=="35", SCFA == "Valerate") %>% pull(estimate) %>% round(.,3)`, *p^adj^* = `r abund_corr %>% filter(taxon == "Acidaminococcaceae_unclassified (Otu0007)", age=="35", SCFA == "Valerate") %>% pull(q) %>% round(.,3)`). Ruminococcacea_unclassified spp. (OTU0047)
was positively correlated with butyrate at 28 days ($\rho$ = `r abund_corr %>% filter(taxon == "Ruminococcaceae_unclassified (Otu0047)", age == 28, SCFA == "Butyrate") %>% pull(estimate) %>% round(.,3)`, *p^adj^* = `r abund_corr %>% filter(taxon == "Ruminococcaceae_unclassified (Otu0047)", age == 28, SCFA == "Butyrate") %>% pull(q) %>% round(.,3)`). 
A single OTU, Lachnospiraceae_unclassified (OTU0002), was negatively correlated 
with butyrate at 8 dpi ($\rho$ = `r abund_corr %>% filter(taxon == "Lachnospiraceae_unclassified (Otu0002)", age==28, SCFA == "Butyrate") %>% pull(estimate) %>% round(.,3)`, *p^adj^* = `r abund_corr %>% filter(taxon == "Lachnospiraceae_unclassified (Otu0002)", age==28, SCFA == "Butyrate") %>% pull(q) %>% round(.,3)`). 
...
propionate_28_mM <- pw %>% 
                       ...
                       unnest(data) %>% 
                       group_by(cohort) %>% 
                       summarise(mean_ppm = mean(Concentration)) %>%
                       mutate(mean_mM = ((mean_ppm/1000)/propionate_MolWt)*1000)
...
mock-challeng controls. However, at 8 dpi the levels of propionate detected in 
*Salmonella*-challenged chickens were greater in chickens in the jGOS group 
(`r round(propionate_28_mM[[4,3]],3)` mM) relative to the chickens on the 
control diet (`r round(propionate_28_mM[[2,3]],3)` mM). As supplementary GOS was 
...
content, it is possible that the inimical effect of propionate heightened in 
the pH of the cecum or the niche occupied by *Salmonella*. *S*. Enteritidis 
P125109 has an intact operon that can in principle breakdown propionate via the 
...
processes including *prpR* - propionate catabolism operon regulatory protein 
...
*P. faecium* has been shown to produce propionate through the catabolism of 
...
concentration.*Megamonas* spp. is also known to metabolize succinate and produce propionate 
[@polansky2016processes; @barnes1979factors; @sakon2008sutterella; @chevrot2008rupellensis].
Khan and Chousalkar [@khan2020salmonella] observed chickens dosed with a probiotic 
...
from the cecal content of all chickens and all rarefaction curves reached 
...
of *Salmonella* infection, and the interaction with a narrow category of specific 
...
**(B)** Mean bird mass for each of 7 pens. The number above bar indicates *p*-value after adjustment 
for the multiple comparisons per timepoint as determined using Tukey Honest Significant 
Difference method. The dashed line indicates breed performance objective for male 
Ross 308 chickens (2014). 
**(C)** Mean ileal villus length per individual bird collected from each independent pen
(*n* = `r min(villus_n$n)`). The number above bar indicates *p*-value determined 
using Tukey Honest Significant Difference method with adjustment for the multiple 
...
**(A)** PCoA of Bray-Curtis dissimilarities (r^2^ = 0.55%). Each data marker 
...
**(B)** Alpha diversity as represented by inverse Simpson index. Each data marker 
...
followed by days post infection in parentheses. The number above bar indicates 
p-value, determined independently for each timepoint using Dunn's 
Kruskal-Wallis Multiple Comparisons (Benjamini-Hochberg). Only *p*-values < 0.05 
are shown. The fill color indicates the diet at the time of sampling: unfilled, 
...
scalr and `df %>% ungroup |> slice_min(chal.max) |> pull(taxon)` excluded for 
clarity. Superfluous numeric ID strings are also removed for clarity. OTUs were
...
**(F)** Effect of S. Enteritidis challenge on expression of IFN-$\gamma$, IL-17A, 
IL-1$\beta$, IL-22, IL-8L1 and IL8-L2 in the ceca. Gene expression calculated 
...
adjusted p-values < 0.05 are shown. Test shaded strips above each panel indicate
...
error bars indicate the upper (0.975) and lower (0.025) quantiles. Statistical 
...
(*p*^adjust^ $\leq$ 0.05). Results are reported at OTU taxonomic 
...
Effect of *S.* Enteritidis challenge on cecal expression of IL-4, STAT6, STAT5B, 
STAT3, IL-10, NOS2, STAT4, GATA3, IL-15, TGIF1. Gene expression calculated as 
...
time point with the Benjamini-Hochberg method. Only adjusted p-values < 0.05 are 
shown. Test shaded strips above each panel indicate the corresponding gene name. 
The shaded strip areas indicate the gene names. 
...
(*p*^adjust^ $\leq$ 0.05). Results are reported at OTU taxonomic levels. 
...
The table shows the proportion (%) of liver and spleen tissues collected from 
challenged chickens from which *Salmonella* could be isolated. The number of
...
using different batches of DNA isolation kit reagents. The figures in parenthesis 
indicate the rank of the read abundance for that OTU compared to all `r nrow(OTU_raw)` 
communities surveyed, with `r nrow(OTU_raw)` being the highest rank and 1 the 
lowest. Tied values are jointly assigned the highest rank.
